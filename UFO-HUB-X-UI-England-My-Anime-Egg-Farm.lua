...
--===== UFO HUB X • Home – Model A V1 + AA1 Auto Collect + Hold Box Stack + Relay 3s (GLOBAL RUNNER) =====
-- Loop: CollectEarnings -> Hold/Wait 5s (ensure holding) -> Relay 3s (ensure holding) -> repeat
-- Fix: เปิดสวิตช์ค้างไว้ แล้วรัน UI หลักใหม่ = ทำงานต่อทันที (ไม่ต้องกดเข้า Home)
...
do
    ...
    local SYSTEM_NAME = "AutoCollectHoldBoxStack"
    ...
    -- CollectEarnings
    local cachedCollectRemote
    local function GetCollectRemote()
        if cachedCollectRemote and cachedCollectRemote.Parent then
            return cachedCollectRemote
        end
        cachedCollectRemote =
    ...
    local STATE = {
        ...
        CycleWait = SaveGet("CycleWait", 5), -- wait/hold 5s
        RelaySec  = SaveGet("RelaySec", 3),  -- ✅ relay 3s
    }
    ...
    local function applyFromState()
        ...
        task.spawn(function()
            while STATE.Enabled and loopToken == myToken do
                ...
                -- 2) ถือของ + รอ 5 วิ (คุมให้ถืออยู่)
                ensureHolding()
                local w = tonumber(STATE.CycleWait) or 5
                if w < 0.2 then w = 0.2 end
                local tHoldEnd = os.clock() + w
                while STATE.Enabled and loopToken == myToken and os.clock() < tHoldEnd do
                    ensureHolding()
                    task.wait(0.25)
                end

                -- 3) รีเลย์ 3 วิ (คุมให้ถืออยู่)
                local r = tonumber(STATE.RelaySec) or 3
                if r < 0 then r = 0 end
                if r > 0 then
                    local tRelayEnd = os.clock() + r
...
----------------------------------------------------------------------
-- 2) UI PART: Model A V1 ใน Tab Home (Row เดียว) + sync
----------------------------------------------------------------------
registerRight("Home", function(scroll)
    local TweenService = game:GetService("TweenService")
    local AA1 = _G.UFOX_AA1 and _G.UFOX_AA1["AutoCollectHoldBoxStack"]
    ...
    for _, name in ipairs({"CH_Header","CH_Row1"}) do
    ...
    header.Text = "Auto Collect + Hold Box Stack 💸📦"
    ...
    local setVisual = makeRowSwitch("CH_Row1", base + 2, "Auto Collect + Hold Box Stack", function()
